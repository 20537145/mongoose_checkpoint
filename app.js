...
app.delete('/delete/:id',async(req,res)=>{
    try{
        const {id} = req.params
        const person = await PersonSchema.findByIdAndDelete(id); // remove delete zoz ya9dhio :)
        res.status(200).send(person);
    }
    catch(e){
        res.send({message:e.message})
...
app.delete('/delete/',async(req,res)=>{
    try{
        const data = req.body
        const person = await PersonSchema.deleteMany(data); // remove delete zoz ya9dhio :)
        res.status(200).send(person);
    }
    catch(e){
        res.send({message:e.message})
